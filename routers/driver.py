...
def accept_delivery(delivery_id: int, driver_email: str = Cookie(None), db: Session = Depends(get_db)):
    ...
    # Safely attempt to claim the job (prevents race conditions)
    result = db.execute(
        text("""
...
def complete_delivery(delivery_id: int, driver_email: str = Cookie(None), db: Session = Depends(get_db)):
    ...
    # Find the delivery to verify ownership and get the booking_id
    delivery = db.execute(
    ...
    if delivery:
        ...
        # Free up the driver for the next job
        db.execute(
            text("UPDATE delivery_drivers SET is_available = 1 WHERE id = :driver_id"),
            {"driver_id": driver.id}
        )
        
        # Automatically complete the core Booking
        db.execute(
